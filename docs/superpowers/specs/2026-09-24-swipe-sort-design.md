...
- Quitting from the pause screen discards the current round entirely, including its points and any lives it cost, keeps the rounds already finished, marks the run incomplete and returns to Home without a results screen. An incomplete run's score is therefore always the sum of its recorded rounds.
...
- Switch cost: for each round with at least 6 correct items that have a reaction time (held items have none), the mean reaction time of the first 3 of them minus the mean reaction time of the rest. The run's switch cost is the mean across qualifying rounds, or nil if no round qualifies.
...
- `RunState`: a value type with `mutating func apply(_ event: RunEvent, at now: Duration) -> [RunEffect]`. Deadlines are applied before the event: any event arriving at or after the round clock's expiry ends the round first, with the item on screen cancelled without penalty even if its own deadline had also passed (a late tick errs in the player's favour); otherwise an answer arriving at or after the item deadline resolves as a timeout. Correctness never depends on tick ordering. Events: start run, start round, answer(edge), tick, pause, resume, quit. Effects: round intro, round started, item shown, item resolved (with its `ItemResult` and outcome), lives changed, score changed, round ended (with its `RoundResult`), run ended (with its `RunSummary`), feedback(cue), and wake(at:). `tick` is the generic "time may have passed" event: it handles item timeouts, the end of the inter-item transition and round clock expiry. `wake(at:)` tells the driver the next time it must send a tick, always the earliest of the item deadline, the transition end and the round deadline. All timing is computed from the `now` passed in; the engine owns no clock.
